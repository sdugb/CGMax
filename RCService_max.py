...
class DAMService():
    ...
    def __init__(self):
        result = self.send(RCConfig_max.myRootURL + '/team/getTeamInfo', {'name': RCConfig_max.teamName})
        if result == '0' or result == '1':
            QtGui.QMessageBox.information(self, u"错误信息", u"社区应用服务器未开或网络不通!!!")
            #cmds.confirmDialog(title=u'错误', message=u'社区应用服务器未开或网络不通')
            self.status = False
        elif not result:
            str = u'团队不存在--' + RCConfig_max.teamName
            QtGui.QMessageBox.information(self, u"错误信息", str)
            #cmds.confirmDialog(title=u'错误', message=str)
            self.status = False
        else:
            team = json.loads(result)
            print 'teamName =', team['teamName']
            serviceList = team['service']
            RCConfig_max.teamDBName = team['teamDBName']
            for service in serviceList:
                if service['name'] == RCConfig_max.teamName + '-designfs':
                    self.myDAMURL = service['url']
                    break
            print u'应用服务器 =', self.myDAMURL
            self.status = True
        #self.url = RCConfig_max.myURL
        #self.myDAMURL = RCConfig_max.myDAMURL
    ...
    def login(self, name, password, b = True):
        m = hashlib.md5()
        #m.update(password)
        m.update('gbzz01')
        #m.update('sdt1234')
    ...
    def myWFGetProjectTask(self, projectName):
        return self.send(self.myDAMURL + '/myWF/getTasksOfProject', {'projectName': projectName})

    def myWFGetParentProjectInfo(self, projectName):
        return self.send(self.myDAMURL + '/myWF/getParentProjectInfo', {'projectName': projectName})
    ...
    def myWFSubmitTask(self, task):
        return self.send(self.myDAMURL + '/myWF/submitTask', {'taskName': task['name'], 'projectName': task['projectName']})
    ...
    """
    def myWFSetTaskAsset(self, taskName, projectName, Url, folderID, assetID):
        return self.send(self.myDAMURL + '/MyWF/setTaskAsset', {'taskName': taskName, 'projectName': projectName,
                                                                ...
                                                                'assetID': assetID})
    """
    def myWFSetTaskAsset(self, id, taskName, projectName, Url,  assetID, textureFolderID, note, assetVersion, plugin, frameList, upfolderid):
        return self.send(self.myDAMURL + '/MyWF/setTaskAsset', {'id': id, 'taskName': taskName, 'projectName': projectName,
                                'IconUrl': Url, 'assetID': assetID, 'textureFolderID': textureFolderID,
